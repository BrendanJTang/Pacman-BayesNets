...
def fillObsCPT(bayesNet, gameState):
    ...
    "*** YOUR CODE HERE ***"
    #unconditioned variable is the obsPos; conditioned variable is the foodHouse and ghostHouse location
    
    mapper = {}
    mapper[BOTTOM_LEFT_VAL] = bottomLeftPos
    mapper[TOP_LEFT_VAL] = topLeftPos
    mapper[BOTTOM_RIGHT_VAL] = bottomRightPos
    mapper[TOP_RIGHT_VAL] = topRightPos

    #does switched columns matter? in the autograder test
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            obsVar = OBS_VAR_TEMPLATE % obsPos
            #create Factpr
            #what are possible assignments
            #set proper probability
            ...
            for assignment in obsFactor.getAllPossibleAssignmentDicts():
                if mapper[assignment[FOOD_HOUSE_VAR]] == housePos:
                    if assignment[obsVar] == 'red':
                        ...
                        prob = 0
                elif mapper[assignment[GHOST_HOUSE_VAR]] == housePos:
                    if assignment[obsVar] == 'red':
                        prob = PROB_GHOST_RED
                    elif assignment[obsVar] == 'blue':
    ...
    # for bottomLeftWall in bottomLeftWalls:
    #     obsFactor = bn.Factor([bottomLeftWall], HOUSE_VARS, bayesNet.variableDomainsDict())
    #     for assignment in obsFactor.getAllPossibleAssignmentDicts():
    ...
    #             prob = 1
    #         obsFactor.setProbability(assignment, prob)
    #     bayesNet.setCPT(bottomLeftPos, obsFactor)

    # for topLeftWall in topLeftWalls:
    #     obsFactor = bn.Factor([topLeftWall], HOUSE_VARS, bayesNet.variableDomainsDict())
    #     for assignment in obsFactor.getAllPossibleAssignmentDicts():
    ...
    #             prob = 1
    #         obsFactor.setProbability(assignment, prob)
    #     bayesNet.setCPT(topLeftPos, obsFactor)

    # for bottomRightWall in bottomRightWalls:
    #     obsFactor = bn.Factor([bottomRightWall], HOUSE_VARS, bayesNet.variableDomainsDict())
    #     for assignment in obsFactor.getAllPossibleAssignmentDicts():
    ...
    #             prob = 1
    #         obsFactor.setProbability(assignment, prob)
    #     bayesNet.setCPT(bottomRightPos, obsFactor)

    # for topRightWall in topRightWalls:
    #     obsFactor = bn.Factor([topRightWall], HOUSE_VARS, bayesNet.variableDomainsDict())
    #     for assignment in obsFactor.getAllPossibleAssignmentDicts():
    ...
    #             prob = 1
    #         obsFactor.setProbability(assignment, prob)
    #     bayesNet.setCPT(topRightPos, obsFactor)

    #q:how to check if there is a ghost house, food house, or no house at a position?
    #a: you're not. You're merely filling in the table based on every possible assignment. Look at fillHouseCpt for
    #tips.
    #Q:what variable do we use when setting a factor? food house var, ghost house var? what if no house?
    #A:use 'obsVar'. See first response by Luowen under piazza post titled 'Q1-2'.
    #use PROB_FOOD_RED and PROB_GHOST_RED from top of file, set bayesNet.setCPT for each factor
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        topLeft = {}
        topLeft.update(evidence)
        topLeft[FOOD_HOUSE_VAR] = TOP_LEFT_VAL
        topLeft[GHOST_HOUSE_VAR] = TOP_RIGHT_VAL
        topRight = {}
        topRight.update(evidence)
        topRight[FOOD_HOUSE_VAR] = TOP_RIGHT_VAL
        topRight[GHOST_HOUSE_VAR] = TOP_LEFT_VAL

        leftExpectedValue = cpt.getProbability(topLeft) * WON_GAME_REWARD + cpt.getProbability(topRight) * GHOST_COLLISION_REWARD
        rightExpectedValue = cpt.getProbability(topLeft) * GHOST_COLLISION_REWARD + cpt.getProbability(topRight) * WON_GAME_REWARD
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
        probsOutcomes = self.getExplorationProbsAndOutcomes(evidence)
        for(prob, ev) in probsOutcomes:
            expectedValue += prob * max(self.computeEnterValues(ev, enterEliminationOrder))
