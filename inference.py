...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        for join in eliminationOrder:
            factorList, joinedFactor= joinFactorsByVariable(factorList, join)
            if len(joinedFactor.unconditionedVariables()) > 1:
                eliminated = eliminate(joinedFactor, join)
                factorList.append(eliminated)
        joinFactor = joinFactors(factorList)
        normalized = normalize(joinFactor)
        return normalized
