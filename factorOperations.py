...
def joinFactors(factors):
    ...
    "*** YOUR CODE HERE ***"
    unionOfUnconditioned = reduce(lambda x, y: x | y, setsOfUnconditioned)
    
    setsOfConditioned = [set(factor.conditionedVariables()) for factor in factors]
    unionOfConditioned = reduce(lambda x, y: x | y, setsOfConditioned)
    
    for unconditionedVar in unionOfUnconditioned:
    	if unconditionedVar in unionOfConditioned:
    		unionOfConditioned.remove(unconditionedVar)

    newFactor = Factor(unionOfUnconditioned, unionOfConditioned, factors[0].variableDomainsDict())
